...
MODIFIED VERSION: Uses enhanced Canny edge detection instead of original Scharr method
Enhanced parameters: sigma=1, low_threshold=3, high_threshold=18, closing_size=6, erosion_after=1
...
from skimage import morphology, measure, restoration, color
from skimage.filters import scharr, gaussian
from skimage.feature import canny  # Added for enhanced Canny detection
import numpy as np
from scipy import ndimage
...
def enhanced_canny_edge_detection(gray):
    """
    Enhanced Canny edge detection with optimized parameters for plankton images
    Uses the parameters you developed: sigma=1, low_threshold=3, high_threshold=18, 
    closing_size=6, erosion_after=1
    """
    # Apply Canny edge detection with optimized parameters
    edges = canny(gray, sigma=1, low_threshold=3, high_threshold=18)
    
    # ENHANCED GAP FILLING - multiple morphological operations
    edges = morphology.closing(edges, morphology.square(3))
    edges = morphology.closing(edges, morphology.disk(2))  
    edges = morphology.closing(edges, morphology.square(6))  # closing_size=6
    ...
    # Final erosion
    edges = morphology.erosion(edges, morphology.square(1))  # erosion_after=1
    
    # Remove small objects
    edges = morphology.remove_small_objects(edges, 50)  # min_obj_size=50
...
def quick_features(img,save_to_disk=False,abs_path='',file_prefix='',cfg = []):
    ...
    if cfg:
        ...
        deconv_iter = cfg.get("DeconvolveIterations",13)
        deconv_mask_weight = cfg.get("DeconvolveMaskWeight",0.7)
        edge_thresh = cfg.get('EdgeThreshold',2.5)  # Not used in Canny version
        use_jpeg = cfg.get("UseJpeg").lower() == 'true'
        raw_color = cfg.get("SaveRawColor").lower() == 'true'
        # Note BGR rather than RGB for opencv compat
        gains = [
        ...
        use_jpeg = False
        raw_color = True
        edge_thresh = 2.5  # Not used in Canny version
        deconv_method = 'um'
        deconv_iter = 13
        deconv_mask_weight = 0.7
    ...
    # MODIFIED SECTION: Replace original Scharr edge detection with enhanced Canny
    # Original code was:
    # edges_mag = scharr(gray)
    # edges_med = np.median(edges_mag)
    # edges_thresh = edge_thresh*edges_med
    ...
    # NEW: Enhanced Canny edge detection with your optimized parameters
    edges = enhanced_canny_edge_detection(gray)

    bw_img = edges

    # Compute morphological descriptors (rest remains unchanged)
    label_img = morphology.label(bw_img,background=0)
    props = measure.regionprops(label_img,gray)
