...
def process_image(bundle):
    ...
    img_c_8bit = cvtools3.convert_to_8bit(image)

    # images will be saved out later so set save_to_disk to False
    features = cvtools3.quick_features(
        img_c_8bit,
        save_to_disk=False,
        abs_path=image_dir,
        file_prefix=prefix,
...
def run(data_path,cfg):
    ...
    for index, image in enumerate(image_list):
        ...
        if 'bayer_conv' in locals():
            bundle['image'] = cvtools3.import_image(os.path.dirname(image),filename,bayer_pattern=bayer_conv)
        else:
            bundle['image'] = cvtools3.import_image(os.path.dirname(image),filename)
        ...
        bundle_queue.append(bundle)
        print ("Loading images... (" + str(index) + " of " + str(total_images) + ")\n"),
        # except:
        #     print('some kind of tomerror')
        #     continue
        #if index > 2000:
        #    total_images = index
        #    break
    ...
    #ignore the CPU properties, and just use 16 threads in case lots of them hang
    n_threads = 32
            ...
            for directory in directory_list:
                ...
                if os.path.isdir(directory):
                    for data_subdir in data_subdirs:
                        if data_subdir == '0p5x':
                            # load the config file
                            cfg = xmlsettings.XMLSettings(os.path.join(sys.path[0],'settings0p5x.xml'))
                            print ("Settings file: " + os.path.join(sys.path[0],'settings0p5x.xml'))
                        else:
                            # load the config file
                            cfg = xmlsettings.XMLSettings(os.path.join(sys.path[0],'settings5p0x.xml'))
